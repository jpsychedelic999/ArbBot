...
const determineProfitability = async (_routerPath, _token0Contract, _token0, _token1) => {
  ...
  try {
    ...
    console.log(`Estimated amount of WETH needed to buy enough Metis on ${exchangeToBuy}\t\t| ${ethers.formatUnits(estimate[0], 'ether')}`)
    console.log(`Estimated amount of WETH returned after swapping Metis on ${exchangeToSell}\t| ${ethers.formatUnits(result[1], 'ether')}\n`)
